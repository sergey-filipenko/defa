...
<Workbook xmlns="urn:schemas-microsoft-com:office:spreadsheet"
    ...
    <Worksheet ss:Name="Ark1">
        <Table ss:ExpandedColumnCount="10" ss:ExpandedRowCount="11" x:FullColumns="1"
...
<Row ss:AutoFitHeight="0">
    <Cell ss:Index="2"><Data ss:Type="String">{{replacement}}</Data></Cell>
...
EOS;
    ...
    static public function getXml($data) {
        ...
        foreach ($data['product'] as $product) {
            $product['replacement'] = $product['replacement'] == '1'?'YES':'NO';
            $productData.= str_replace(array('{{id}}', '{{rowIndex}}'), array($i, $rowInd), self::$productHeader) . str_replace(
                    array('{{replacement}}', '{{sale_date}}', '{{manufacturer}}', '{{year}}', '{{type}}', '{{number}}',
                        '{{owner}}', '{{mounted_by}}', '{{comment}}'),
                    array($product['replacement'], $product['sale_date'], $product['manufacturer'], $product['year'], $product['type'],
...
//            mysql_select_db(DB_NAME);
    ...
    static public function getNextId() {
        ...
        $q=self::$conn->prepare("SELECT COUNT(*) as 'count' FROM  `contacts`");
		$q->execute();
    ...
    static public function saveData($contactId, $data) {
        ...
        $contacts_data = array(
            ...
            'ref_banknum' => $data['ref_banknum'],
            'ref_shipping_comp' => $data['ref_shipping_comp'],
            'ref_comment' => $data['ref_comment']
        );
...
function sendEmail($to, $subject, $filename) {
	...
	$mail->Host = 'smtp.bewide.net';  // Specify main and backup server
	$mail->SMTPAuth = true;      // Enable SMTP authentication
	$mail->Port = 587;
	$mail->Username = 'web@defa.ds.bewide.net';                            // SMTP username
	$mail->Password = 'cP13WLaNve';                           // SMTP password
    
    $mail->From = 'no-reply@defa.com';
    $mail->FromName = 'Defa';
    $mail->addAddress(SUPPORT_EMAIL);  // Add a recipient
//    $mail->addBCC('sergey_f@onix-systems.com');
    $mail->addBCC('denis@onix-systems.com');
    $mail->addBCC('anders.medhus@utforming.no');
    
    $mail->addAttachment($file);
    $mail->Subject = $subject;
    $mail->Body    = 'XML is attached';
    if(!$mail->send()) {
...
//    $mail->addBCC('sergey_f@onix-systems.com');
    $mail->addBCC('denis@onix-systems.com');
    $mail->Subject = $subject;
    $mail->Body    = 'To view request visit "' . PREVIEW_URL . '?contact_id=' . $subject . '"';
    if(!$mail->send()) {
        echo 'Message could not be sent.';
        echo 'Mailer Error: ' . $mail->ErrorInfo;
        exit;
